...
class Timpanometria extends CI_Controller {
    ...
    public function timpanometriaJSON($id = NULL) {
        verificar_sessao($this->session);
        $this->output->enable_profiler(false);
        $dados['timpanometria']['OD'] = $this->timpanometria->getCompletarImpedanciometriaOD($id);
        $dados['timpanometria']['OE'] = $this->timpanometria->getCompletarImpedanciometriaOE($id);
        echo json_encode( $dados['timpanometria']);
