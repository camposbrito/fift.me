...
// Valores eixo VALE
$rs = $obj->listar ( "eaf.Simbolo, ea.*", "MD5(Consulta) = '" . $_GET ['Consulta'] . "' AND eaf.Cod in (select Cod from exameaudiometricofaixa eaf1 WHERE eaf1.Simbolo in ('VALD', 'VAMLD', 'VOMLD', 'VOSMLD'))", array (
...
function PopulaEixos($Field, $Lista) {
	$i = 0;
	global $EixoConectores_A_VAML, $EixoConectores_A_VAL, $EixoConectores_Eixo, $EixoConectores_Vlr, $EixoConectores, $EixoX, $VALD, $VAMLD, $VOMLD, $VOSMLD, $VALD_A, $VAMLD_A, $VOMLD_A, $VOSMLD_A, $VALD_E, $VAMLD_E, $VOMLD_E, $VOSMLD_E;
...
for($j = 0; $j <= 8; $j ++) {
	if (($EixoConectores_Vlr [$j] != null) and (${"EixoConectores_A_VAL"} [$j] == 'N') and (${"EixoConectores_A_VAML"} [$j] == 'N')) {
		...
		if ($Debug)
			echo $j . " < <br>";
		// $i++;
	} else if ((${"EixoConectores_A_VAL"} [$j] == 'S') or (${"EixoConectores_A_VAML"} [$j] == 'S')) {
		if ($Debug)
			echo $j . " << <br>";
...
if (isset ( ${'EixoConectores_Eixo' . $i} )) {
	...
	$graph->Add ( ${'sp1_Conectores' . $i} );
	${'sp1_Conectores' . $i}->SetColor ( 'red' );
	${'sp1_Conectores' . $i}->SetStyle ( "solid" );
	$graph->img->SetAntiAliasing ( false );
